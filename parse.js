...
var parse = cadence(function (async, arguable, stdin, stderr, consumer) {
    require('assert')(arguments.length == 5)
    ...
    async(function () {
        ...
        if (state.code != null) {
            arguable.exitCode = state.code
        } else {
            arguable.exitCode = (!count || eof) ? 0 : 1
        }
        return []
    })
