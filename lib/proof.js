...
Test.prototype.bailout = function (error) {
  ...
  if (process.stdout.write(message)) bailout()
  else process.stdout.once("drain", bailout.bind(this))
  function bailout() {
    if (detail == null || this.__proof__comment(detail)) exit();
...
Test.prototype.callback = function () {
  ...
  if (!varg.length || (varg.length == 1 && typeof varg.length[0] == "string")) {
    var name = varg.shift();
    this.__proof__callbacks.count++;
    return (function (error, result) {
      if (error) {
        if (this.__proof__steps.length && ~this.__proof__steps[0][0].parameters.indexOf("error")) {
      ...
      if (!--this.__proof__callbacks.count) {
        for (key in this.__proof__callbacks.results) {
          if (this.__proof__callbacks.results[name].length == 1) {
            this.__proof__callbacks.results[name] = this.__proof__callbacks.results[name][0];
          }
        }
        this.__proof__invoke();
      }
