...
function execute (expected, setup, run) {
  var actual = 0
    ...
    , cleaning = false
    , untidy
    , cadence = require('cadence')({ wrap: { cleanup: janitor } })
    ;

  if (setup.length) run.unshift(function (step) {
    step.apply(null, setup);
  ...
  function tidy (step) {
    cleaning = true;
    step(function () { step.apply(null, janitors) }, function () { step.apply(null, exit) });
  }
