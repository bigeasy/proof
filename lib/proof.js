...
Test.prototype.callback = function (name) {
  ...
  return (function (error, result) {
    ...
    if (name) {
      this._callbacks.results[name] || (this._callbacks.results[name] = []);
      this._callbacks.results[name].push(splat.shift());
    }
...
Test.prototype._invoke = function () {
  ...
  for (key in (this._callbacks.results || {})) {
    value = this._callbacks.results[key];
  ...
  this._callbacks = { count: 0, results: {} };
  try {
    result = step.apply(this, args);
    if (!(done || this._callbacks.count)) {
      if (typeof result == "object") {
        for (var key in result) {
          this.context[key] = result[key];
        }
...
function execute (test, steps) {
  console.log(test, steps);
  test.__proof__steps = [ steps, [] ];
  untidy = process.env.PROOF_NO_CLEANUP || process.env.UNTIDY;
  untidy = untidy && /^(0|no|false)$/.test(untidy);
  if (untidy) {
    test.__proof__steps[1].push(function () { process.exit() });
  }
  test._invoke();
};
...
module.exports = function () {
  ...
  if (typeof outer[0] == "number") {
    execute(new Test(outer.shift()), outer);
  } else {
    return function () {
      var inner = __slice.call(arguments, 0);
      execute(new Test(inner.shift()), outer.concat(inner));
    };
  }
};
